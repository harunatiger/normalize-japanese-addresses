...
var japaneseAddressesV2 = require('@geolonia/japanese-addresses-v2');
...
const kan2num = (string) => {
    const kanjiNumbers = japaneseNumeral.findKanjiNumbers(string);
...
const getCityRegexPatterns = (pref) => {
    ...
    cities.sort((a, b) => {
        return japaneseAddressesV2.cityName(a).length - japaneseAddressesV2.cityName(b).length;
    });
    const patterns = cities.map((city) => {
        const name = japaneseAddressesV2.cityName(city);
...
const getTowns = (prefObj, cityObj, apiVersion) => __awaiter(undefined, undefined, undefined, function* () {
    const pref = japaneseAddressesV2.prefectureName(prefObj);
    const city = japaneseAddressesV2.cityName(cityObj);
...
function fetchSubresource(kind, pref, city, row, apiVersion) {
    return __awaiter(this, undefined, undefined, function* () {
        const prefN = japaneseAddressesV2.prefectureName(pref);
        const cityN = japaneseAddressesV2.cityName(city);
...
const getRsdt = (pref, city, town, apiVersion) => __awaiter(undefined, undefined, undefined, function* () {
    ...
    const parsed = yield fetchFromCache(`住居表示-${pref.code}-${city.code}-${japaneseAddressesV2.machiAzaName(town)}`, () => __awaiter(undefined, undefined, undefined, function* () {
...
const getChiban = (pref, city, town, apiVersion) => __awaiter(undefined, undefined, undefined, function* () {
    ...
    const parsed = yield fetchFromCache(`地番-${pref.code}-${city.code}-${japaneseAddressesV2.machiAzaName(town)}`, () => __awaiter(undefined, undefined, undefined, function* () {
...
const isKanjiNumberFollewedByCho = (targetTownName) => {
    ...
    if (!xCho)
        return false;
    const kanjiNumbers = japaneseNumeral.findKanjiNumbers(xCho[0]);
...
const getTownRegexPatterns = (pref, city, apiVersion) => __awaiter(undefined, undefined, undefined, function* () {
    return fetchFromCache(`${pref.code}-${city.code}`, () => __awaiter(undefined, undefined, undefined, function* () {
        ...
        const townSet = new Set(pre_towns.map((town) => japaneseAddressesV2.machiAzaName(town)));
        const towns = [];
        ...
        for (const town of pre_towns) {
            towns.push(town);
            const originalTown = japaneseAddressesV2.machiAzaName(town);
        ...
        // 少ない文字数の地名に対してミスマッチしないように文字の長さ順にソート
        towns.sort((a, b) => {
            let aLen = japaneseAddressesV2.machiAzaName(a).length;
            let bLen = japaneseAddressesV2.machiAzaName(b).length;
            // 大字で始まる場合、優先度を低く設定する。
            // 大字XX と XXYY が存在するケースもあるので、 XXYY を先にマッチしたい
            if (japaneseAddressesV2.machiAzaName(a).startsWith('大字'))
                aLen -= 2;
            if (japaneseAddressesV2.machiAzaName(b).startsWith('大字'))
                bLen -= 2;
            return bLen - aLen;
        });
        const patterns = towns.map((town) => {
            const pattern = toRegexPattern(japaneseAddressesV2.machiAzaName(town)
                ...
                if (match.match(/^壱/)) {
                    ...
                    const num = match
                        ...
                        return japaneseNumeral.kanji2number(match).toString();
                    })
        ...
        for (const town of towns) {
            const chomeMatch = japaneseAddressesV2.machiAzaName(town).match(/([^一二三四五六七八九十]+)([一二三四五六七八九十]+)(丁目?)/);
            if (!chomeMatch) {
                continue;
            }
    ...
    return __awaiter(this, undefined, undefined, function* () {
        ...
        if (town.rsdt) {
            ...
            for (const rsdt of res) {
                const addrPart = japaneseAddressesV2.rsdtToString(rsdt);
                if (match[0] === addrPart) {
                    return {
                        rsdt,
            ...
            for (const chiban of res) {
                const addrPart = japaneseAddressesV2.chibanToString(chiban);
                if (match[0] === addrPart) {
                    return {
                        chiban,
                        rest: addr.substring(addrPart.length),
...
const normalize$1 = (address_1, ...args_1) => __awaiter(undefined, [address_1, ...args_1], undefined, function* (address, _option = defaultOption) {
    ...
    other = patchAddr(pref ? japaneseAddressesV2.prefectureName(pref) : '', city ? japaneseAddressesV2.cityName(city) : '', town ? japaneseAddressesV2.machiAzaName(town) : '', other);
    if (pref)
        level = level + 1;
    if (city)
        level = level + 1;
    ...
    if (option.level <= 3 || level < 3) {
        const result = {
            pref: pref ? japaneseAddressesV2.prefectureName(pref) : undefined,
            city: city ? japaneseAddressesV2.cityName(city) : undefined,
            town: town ? japaneseAddressesV2.machiAzaName(town) : undefined,
            other: other,
            level,
            point,
            metadata: {
    ...
    if (normalizedAddrPart.rsdt) {
        addr = japaneseAddressesV2.rsdtToString(normalizedAddrPart.rsdt);
        ...
        addr = japaneseAddressesV2.chibanToString(normalizedAddrPart.chiban);
        other = normalizedAddrPart.rest;
...
const requestHandlers = {
    file: (fileURL, options) => __awaiter(undefined, undefined, undefined, function* () {
        ...
        const f = yield node_fs.promises.open(filePath, 'r');
        let contents, ok;
        if (typeof o.length !== 'undefined' && typeof o.offset !== 'undefined') {
    ...
    http: (fileURL, options) => {
        ...
        return undici.fetch(fileURL.toString(), {
            headers,
        });
    },
};
...
//# sourceMappingURL=main-node-cjs.cjs.map
